...
def run(image):
    ...
    feature_ext = AutoFeatureExtractor.from_pretrained("microsoft/beit-base-patch16-224-pt22k-ft22k")
    feat = feature_ext(image, return_tensors='pt')

    os.system(f"echo '{feat}' >> features")
    # Pass through model
...
if __name__ == '__main__':
    image = Image.open('playground/light-yagami-1.jpg').convert('RGB')
    conf = run(image)

    op = {
        score:name for name,score in conf.items()
    }

    max_score = max(op.keys())
